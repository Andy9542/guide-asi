...
Охват считается по каталогу на хосте: каждый файл профиля (JS_EXTS) обязан оказаться
в `paths.scanned`, любая запись в `errors[]` отменяет вердикт. Пробел обхода
(недоступный подкаталог, симлинк) и испорченный отчёт — данные, не исключение: они
копятся в списке и печатаются в строке вердикта, чтобы подтверждённая находка могла
их перекрыть (приоритет 1 > 3).

stdout — ровно одна строка `scan_result: …`, всё остальное в stderr. Коды:
  0  ЧИСТО      охват полный, блокирующих находок нет
  1  НАХОДКА    есть ERROR; неполнота, если была, названа в той же строке
  2  неверный вызов
...
# Профиль расширений задан только этой константой: шесть расширений, которые Semgrep
...
# отказа и остаётся на случай смены образа.
...
def inventory(project_dir: str) -> tuple[set[str], list[str]]:
    """Файлы профиля в каталоге и пробелы обхода (что прочитать не удалось).

    Каталог без доступа сузил бы список ожидаемых файлов молча, и сверка с
    `paths.scanned` сошлась бы сама с собой; поэтому on_error пишет пробел. Симлинки
    обход не разрешает, это заявленная граница.
    """
    found, gaps = set(), []
...
def severity(result: object) -> str:
    """Уровень одной находки; запись без него отменяет вердикт."""
...
def error_text(err: object) -> str:
    """Текст записи errors[]. Вердикт решает сам факт записи, текст нужен читателю."""
...
def skip_reasons(report: dict) -> dict:
    ...
    Испорченный список отменяет вердикт: по нему читаются причины отсутствия доступа,
    и молчание превратило бы закрытый каталог в «чисто».
    """
...
def no_access(reasons: dict[str, str], missing: list[str]) -> list[str]:
    ...
    Semgrep не от root сообщает о недоступном каталоге сам, а обход на хосте мог этот
    каталог видеть, и тогда разности expected/scanned для него нет.
    """
...
def incompleteness(expected: set[str], report: dict, walk_gaps: list[str]) -> list[str]:
    ...
    Испорченные `paths.scanned` и `paths.skipped` тоже идут в список: исключение
    отсюда миновало бы ветку «находка важнее неполноты», и ERROR-находка при rc=1
    пряталась бы за «НЕПОЛНО».
    """
    gaps = list(walk_gaps)
...
def where(result: dict) -> str:
    """`check_id в path:line` одной находки; вместо отсутствующего поля стоит «?»."""
...
def classify(expected: set[str], report: dict, rc: int,
    ...
    classify возвращает список находок, чтобы main не фильтровал results второй раз:
    список нужен и для решения, и для печати атрибуции.
    """
    if rc not in (0, 1):
