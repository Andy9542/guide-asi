...
#
# Из /src сканер берёт только lock-файлы; всё, что он читал бы оттуда как настройку,
# выключено: политику задаёт только /rules/osv-scanner.toml (`--config` перекрывает
# osv-scanner.toml проекта на всех уровнях), а .gitignore проекта не прячет lock-файл
# (`--no-ignore`). Новый вход сканера из дерева — новый флаг здесь и строка в selftest.
# Подробности и замеры — README, «Чего не закрывает».
