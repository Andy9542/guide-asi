...
TMP=$(mktemp -d)
# Каталоги для docker лежат в дереве репозитория: docker из snap не видит /tmp хоста,
...
# trap сначала возвращает права: каталог с правами 000 не удалить, не открыв его.
...
# Предусловия: без них прогон заканчивается кодом 3 ИНФРА.
...
# read_pin берёт пины из depscan.sh, чтобы у selftest не было своей копии.
...
# Тот же набор флагов без --disable-nosem: прогон с ним показывает, что флаг несущий.
...
# Semgrep разбирает файл правил и видит оба правила.
...
# depscan отклоняет вредоносный каталог и сканирует вложенную зависимость.
...
# depscan отклоняет уязвимую зависимость в lock-файле.
expect 1 'ОТКЛОНЕНО' -- sh "$HERE/depscan.sh" "$SRC_VUL"
saw 'known vulnerabilities'

# Всё, чем проверяемое дерево отменяло бы свою проверку, в одной копии фикстуры: исключение
# пакета в корне, исключение уязвимости в sub/ со своим lock-файлом, lock-файл в .gitignore.
# Ждём 1 без «Filtered» и оба lock-файла в «Scanned». Копия в $OSVD: testdata/ на ходу не меняем.
...
# depscan: проверять было нечего, отдельный исход 4 без «ЧИСТО».
...
# depscan: демона нет, вердикта нет; «ОТКЛОНЕНО» в выводе быть не должно.
...
# вызов и код 1, оба ожидаемы, и ни одного ERROR про вынос токена.
expect 1 'install-script-network-beacon' -- \
  docker run --rm -v "$HERE:/rules:ro" -v "$SRC_BEN:/src:ro" "$SEMGREP_IMAGE" \
  semgrep scan --metrics=off --config "$RULE" --error /src
never 'install-script-ci-token-exfil'

# Покрытие по файлам: ERROR на каждом из 25 вредоносных образцов, включая тот, что
# лежит в node_modules/, и два с меткой подавления.
...
# собственного кода, и Semgrep блокирует только 23.
...
# Пропуск по размеру: Semgrep не читает файл больше 1 МБ и выходит кодом 0. Утечку в
# конце наполнителя никто не видел, и вердикта нет.
...
# каталога на хосте обрывается, список ожидаемых файлов неполон, вердикта нет.
if [ "$(id -u)" = 0 ]; then
  ...
  # файл дочитывает: находка перекрывает неполноту, приоритет 1 > 3. При rootless docker
  # или user namespaces контейнер закрытый каталог не прочтёт, утечку никто не видел,
  # исход 3. Ветку выбирает uid в контейнере.
  ...
  # Закрытый корень проекта без права входа (000) и без права чтения (111): код 3 ИНФРА,
  # и в выводе нет «НЕЧЕГО ПРОВЕРЯТЬ».
  chmod 000 "$LOCKED"
...
# Профиль расширений задан в scan_result.py. Если образ перестанет брать одно из шести,
# охват станет неполным и строка упадёт: дрейф виден сразу.
rm -rf "$BIG"; mkdir "$BIG"
...
# на вредоносном каталоге «ОТКЛОНЕНО» быть не должно, разбирать отчёт было нечем.
mkdir -p "$TMP/fakepy"
printf '#!/bin/sh\nexit 1\n' >"$TMP/fakepy/python3"
