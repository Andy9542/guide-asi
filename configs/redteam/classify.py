...
В stdout уходит одна строка `REDTEAM_VERDICT=pass|fail|infra`, и только она; её читает
run.sh. Пояснения и чужие тексты ошибок идут в stderr: текст из выгрузки promptfoo не
должен попасть в поток, откуда run.sh берёт вердикт, иначе ответ модели мог бы подделать
строку вердикта.

promptfoo 0.123.0 (версия закреплена в run.sh) отдаёт 100 и при провале проверок, и при
ошибке провайдера, а падение инструмента, ошибка конфига и нехватка памяти дают свои
коды; код процесса classify сверяет с вердиктами проб, но исход по нему не выносит.

Манифест ожиданий обязателен: его строит preflight.py по конфигу до вызова модели
(индексы 0..N-1, идентичность каждой пробы, провайдер, промпт), classify сверяет с ним
выгрузку построчно, а без манифеста отвечает кодом 3.

INFRA — это отсутствие явного булева вердикта у теста или у компонента его проверки, а
не наличие текста ошибки рядом: promptfoo кладёт в `error` и текст проваленной проверки.

Вторая линия профиля пробы работает по выгрузке: `testCase` строки состоит из ключей
...
# Единый источник для обеих линий: preflight.py импортирует отсюда. Типы, у которых в
# 0.123.0 нет пути исполнения пользовательского кода и чей отказ отличим от
# отрицательного решения о модели (мы сверили живым прогоном на echo: компонент несёт
# `assertion.type` и булев `pass`, исключений нет; отказ судьи llm-rubric приходит как
# `metadata.graderError`). Новый тип добавляйте только вместе с проверкой того, как он
# сообщает о сбое своего выполнения.
...
# чужой код или чужой ответ, что бы он ни значил в следующей версии.
TESTCASE_KEYS = frozenset({"vars", "assert", "threshold", "description", "options", "metadata"})
TESTCASE_OPTION_KEYS = frozenset({"provider"})           # судья, и только он
# Ключи, уже приходившие из отчётов аудита: отказ называет и ключ, и подделанный им вердикт.
FOREIGN_KEY_REASONS = {
...
def foreign_key_text(key):
    """Чужой ключ с причиной, если она известна по прежним контрпримерам."""
    why = FOREIGN_KEY_REASONS.get(key)
    return f"{key} — {why}" if why else key


def reject_foreign_case_keys(position, case):
    """testCase вне формы экспорта 0.123.0 для профиля — вердикта нет."""
    extra = sorted(set(case) - TESTCASE_KEYS)
    if extra:
        infra(f"#{position}: в testCase есть {', '.join(map(foreign_key_text, extra))} — "
              "форма пробы не та, что снята с 0.123.0 для профиля")
    options = case.get("options")
    if options is not None and not isinstance(options, dict):
        infra(f"#{position}: testCase.options не отображение ({type(options).__name__})")
    extra = sorted(set(options or {}) - TESTCASE_OPTION_KEYS)
    if extra:
        infra(f"#{position}: в testCase.options есть {', '.join(map(foreign_key_text, extra))} "
              "— кроме судьи provider там ничего быть не должно")
...
def check_test_case(position, row, expected):
    ...
    reject_foreign_case_keys(position, case)
    reject_dynamic_values(position, case)
