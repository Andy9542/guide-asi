...
set -u
# Коды шагов читаются явно: запуск через `sh -e` не должен обрывать сводку на первом
# ненулевом шаге, иначе ИНФРА одного каталога спрячет ПРОВАЛ другого.
set +e
...
# sh -n под dash ловит только синтаксис: двойные квадратные скобки, `function` и
# подстановки bash он молча пропускает (массивы `a=(…)` он отвергает сам). Поэтому рядом —
# эвристика по тексту. Условие bash — двойные скобки с пробелом после них, поэтому шаблон
# не трогает ни POSIX-классы символов `[:cntrl:]` в скобках, ни заголовки секций TOML.
syntax_check() {
  rc=0
  ...
  lb='['
  pat="\\$lb\\$lb[[:space:]]|^[[:space:]]*function[[:space:]]|\\\$\\{[^}]*(//|\\^\\^|,,)"
...
# на всех: нет :latest/@latest и ровно три образа с digest. Четвёртый образ по тегу
# без digest эта проверка не заметит — добавляйте его пином и поднимайте число.
pin_check() {
  rc=0
