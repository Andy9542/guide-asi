...
**Goal:** достигнут: контрпример IA-11 воспроизведён на `2dddc8f` и закрыт, контроли сохранены, smoke → 0; пуш `6c46cd7` → CI run [36004075287](https://github.com/Andy9542/guide-asi/actions/runs/36004075287) success; PR #2 открыт от `main`.

**Invariants:** IV1 (run.sh на конфиге аудита и фикстуре, вторая линия на выгрузке базы, контроль со статическим значением), IV2 (конфиги гайда и прежние случаи 79/57, selftest 51, smoke → 0, пины) подтверждены. PC1 соблюдён.

**Deviations:** см. Verify — счётчик `preflight_test` (7bf45d4), замеры базы в шапке фикстуры и docstring (9cca4f4).

**Deferred:** `providers: ["package:…"]` и любой другой один провайдер-строка принимаются preflight (решение автора: провайдер — доверенный вход); `prompts[0]` с динамическим префиксом отклоняется без имени префикса; сообщение второй линии «провайдер не ответил» на исключении из модуля проверки (исход верный, текст неточен, `classify.py`, `why_missing`).

**Code smells:** `docs/tasks/ia-07-09-fixes.md` AS2 («динамические значения только по `file://`») опровергнут `package:` — исторический протокол, не правится.

**Status:** done — IA-11 закрыт, ревью без находок, CI зелёный на `6c46cd7`; PR #2.
