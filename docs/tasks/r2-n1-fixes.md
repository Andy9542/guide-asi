...
**Goal:** достигнут в части кода и проверок: оба контрпримера автора воспроизведены на `0802baf` и закрыты на `c05e2fa`/`7dabf91`, регрессии на месте (`scan_result_test` 29, `preflight_test` 33, строки selftest на закрытый подкаталог и корень), smoke → 0; пуш `adfdecc` → CI run [35830180095](https://github.com/Andy9542/guide-asi/actions/runs/35830180095) success (строка selftest с закрытым каталогом прошла на раннере не от root — AS1 подтверждена). Итоговая вершина PR — коммит с этим текстом (только task-файл); его run и обновлённое описание — в PR #1.
...
**Status:** done — остаточный R2 и N1 закрыты воспроизводимыми проверками, ревью пройдено, CI зелёный на `adfdecc`; описание PR #1 обновлено после зелёного run итоговой вершины.
