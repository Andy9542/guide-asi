...
**Goal:** достигнут в части кода и проверок: контрпримеры IA-06–IA-08 воспроизведены на `3724cbb` и закрыты на `a985bc8`/`205f0ca` (профиль проверок с отказом до promptfoo, вторая линия в classify, неубывающая шкала часов guard), исправные контроли и пороговое агрегирование сохранены, smoke → 0; пуш `77a9a5a` → CI run [35907668171](https://github.com/Andy9542/guide-asi/actions/runs/35907668171) success. Итоговая вершина PR — коммит с этим текстом (только task-файл); его run и обновлённое описание — в PR #1.
...
**Status:** done — IA-06–IA-08 закрыты воспроизводимыми проверками через настоящий `run.sh` и тесты, ревью без находок после фикс-раунда, CI зелёный на `77a9a5a`; описание PR #1 обновлено после зелёного run итоговой вершины.
