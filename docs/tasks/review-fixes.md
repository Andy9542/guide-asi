...
Result: passed (144 проверки шестью независимыми атакующими 19.09.2026 + повторный прогон после правок; два отложенных пункта закрыты прогоном CI 22.09.2026)

Первый прогон: 122 held, 20 broke, 2 deferred. Все 20 сломанных исправлены пятью коммитами (`4cb4a4f`, `f5d942d`, `63aa1f6`, `472a9e7`, `3b839c9`) и перепроверены свежими пробами — ниже они помечены «broke → held». Полные протоколы атак — в журнале сессии; здесь по одной строке на гипотезу.
...
- CK24 (UK4) — 25 минут не хватит на холодный прогон — held: GitHub Actions run 35494798807 (pull_request, head `ea28bee` до переписывания истории; дерево отличается от `bb7879e` только комментарием в .gitignore) завершился успешно в пределах таймаута.
- CK25 (AS1) — у раннера ubuntu-24.04 нет docker или доступа к ghcr.io — held: тот же run 35494798807 прошёл все девять шагов, включая контейнерные selftest semgrep и opa.
...
Goal: обе половины доказаны — локальный selftest 0 и путь «образа нет» → ИНФРА 3; PR #1 с зелёным run 35494798807. Ревью PR от 22.09.2026 (комментарии автора, R1–R6) требует доработки до слияния — см. Conclusion.
...
Outcome: цель по selftest и CI достигнута — `sh build/selftest.sh` → 0, путь «образа нет» → ИНФРА 3, каждая команда «Как проверить у себя» даёт обещанный код и строку, PR #1 с зелёным run 35494798807. Слияние отложено: ревью PR от 22.09.2026 нашло R1–R6 (nosemgrep выключает детектор; охват скана по одной цели; ложная блокировка `store.request().write`; классификатор не проверяет полноту набора; guard в verify необязателен; ReplayGuard без блокировки) — план доработки в комментарии PR. Ветка: 16 коммитов dbbfaf1..review-fixes (история переписана 22.09.2026: заголовок коммита .gitignore).
...
### Assumptions check
- AS1 — held: run 35494798807 на `ubuntu-24.04` прошёл все шаги, включая pull образов с Docker Hub и ghcr.io (CK25).
...
- UK4 — resolved: холодный прогон на раннере уложился в `timeout-minutes: 25` (CK24).
...
Review findings: находок с уверенностью ≥ 80 нет (`up:reviewer`, диф dbbfaf1..2624dfe). Две структурные оговорки уже записаны: каркас selftest продублирован в четырёх каталогах без проверки идентичности (PC1, CK26); `pin_check` держит число образов литералом 3 (CK19).
